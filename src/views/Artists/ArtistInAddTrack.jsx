...
const ArtistInAddTrack = ({ index, handleDelete, handleSliderChange, trackData }) => {
        ...
        <Grid container item xs={12}>
          ...
          <Grid container item xs={9} direction="column" justifyContent="center" alignItems="center">
            ...
            <Grid item xs={5} sx={nameGridStyle}>
              <Typography sx={nameStyle} noWrap>{trackData.allOtherArtists[index].name}</Typography>
            </Grid>

            <Grid container item xs={4}>

              <Grid item xs={8} sx={switchGridStyle}>
                <LabelledAndColoredSwitch size="small" checked={trackData.allOtherArtists[index].primary} color={"#508062"}
                  onChange={(event) => handleSliderChange(index, event.target.checked)} labelPlacement={"end"}
                  label={trackData.allOtherArtists[index].primary ? "Principal" : "Featuring"} />
              </Grid>
